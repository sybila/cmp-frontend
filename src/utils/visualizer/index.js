...
import * as Data from "./get-data.js"
...
function processData(data) {
  return Immutable.Map(
        { orig: data
        , legendItems:
            Immutable.List(data.legendItems || data.graphsets[0].datasets)
        , datasetsVisibility: Immutable.List(data.datasetsVisibility
...
export class Visualizer extends React.Component {
  constructor(...rest) {
    ...
    this.props.models.forEach((model, i) => {
      if (!model.datasets) {
        (async () => {
          model = await (model.model ? Data.getModel : Data.getExperiment)(model.id)
          
          this.setState({ models: this.state.models.set(i, processData(model)) });
        })();
      }
    });
    
    const processedModels =
        this.props.models.map(a => a.datasets ? processData(a) : loadingData);
  ...
  toggleLegendItem(dataIndex, legendIndex) {
    const currentLegendItems = this.state.models.getIn([ dataIndex, "legendItems" ]);
    const newLegendItems = currentLegendItems.map((a, i) => (i === legendIndex) !== a);
  ...
  toggleDatasetVisibility(dataIndex, datasetIndex) {
    const models = this.state.models.updateIn(
        [ dataIndex, "datasetsVisibility", datasetIndex ], a => a.next);
  ...
  setCurrentGraphset(dataIndex, graphset) {
    const models = this.state.models.setIn([ dataIndex, "legendItems" ], graphset);
  ...
  async setAddData(newAddData) {
    ...
    if (this.state.linkedData === null) {
      ...
      (await Promise.all(this.state.models.map(model => {
        if (model.get("orig").linkedData) return model.get("orig").linkedData;
        
        return (model.model ? Data.findModels : Data.findExperiments)(model.id);
      }))).forEach(modelArr => {
        modelArr && modelArr.forEach(model => {
    ...
    if (typeof linkedData == "number") {
      this.setState(
            { addData: this.state.linkedData[linkedData].model
                  ? addDataEnum.model : addDataEnum.experiment
  ...
  toggleExportData() {
    this.setState({ exportData: !this.state.exportData, addData: addDataEnum.none });
  }
  
  exportData() {
    const exportPng = () => {
      Plotly.downloadImage(this.state.plotDivId, { format: 'png', width: 2400, height: 1800, filename: "plot" })
    }
    
    if (this.state.exportFormat == "png") {
       if (this.state.exportWhole && (this.state.x !== null || this.state.y !== null)) {
         this.setXY(null, null, () => setTimeout(exportPng, 80));
       } else exportPng();
      
    } else if (this.state.exportFormat) Data.doExport(this.state.exportFormat,
  ...
  async addData() {
    const fn = this.state.addData === addDataEnum.model
        ? Data.getModel : Data.getExperiment;
  ...
  render() {
    return React.createElement("div", { className: "vis-root", style: { width: this.props.width, height: this.props.height } },
          React.createElement("div", { className: "vis-dialog", style: { minHeight: this.state.addData !== addDataEnum.none || this.state.exportData ? "100%" : "auto" } },
          ...
          this.state.models.map((model, key) => {
            return React.createElement(ModelOptions,
                  { key, modelIndex: key, actions: this.actions, model }
                );
          }),
          React.createElement(XAxisChooser, { state: this.state, actions: this.actions }),
          ...
          React.createElement(Graph,
              { state: this.state
              , actions: this.actions
              , background: this.state.addData !== addDataEnum.none || this.state.exportData
              }),
          React.createElement(GlobalSettings, { state: this.state, actions: this.actions }),
        );
  }
}
...
class XAxisChooser extends React.Component {
  render() {
    ...
    if (this.props.state.models.size == 1) {
      return React.createElement("p", null, "X axis: ",
            React.createElement("select", { onChange, value: this.props.state.xAxis },
              React.createElement("option", { value: "0,0" }, "Time"),
              this.props.state.models.getIn([ "0", "orig" ]).legend.map(
                    (legend, i) => React.createElement("option", { value: "0," + (i + 1), key: i }, legend.name)
                  ),
            ),
          " ",
    ...
    return React.createElement("p", null, "X axis: ",
          React.createElement("select", { onChange, value: this.props.state.xAxis },
            ...
            this.props.state.models.map((model, i) => {
              ...
              return React.createElement("optgroup", { label: m.name, key: i },
                    React.createElement("option", { value: i + ",0" }, "Time"),
                    m.legend.map((legend, ii) => {
                      return React.createElement("option", { value: i + "," + (ii + 1), key: ii }, legend.name);
                    }),
                  );
            }),
          ),
...
class DatasetChooser extends React.Component {
  render() {
    if (this.props.state.xAxis == "" || this.props.state.xAxis.split(",")[1] == "0") return null;
    ...
    return React.createElement("select",
          ...
          datasets.map((dset, i) => React.createElement("option", { key: i, value: i }, dset.name)),
        );
  }
}
...
class DialogAddModel extends React.Component {
  render() {
    return React.createElement("div", { className: "vis-add-model" },
          React.createElement("p", null, "Linked models and experiments: ",
          ...
          React.createElement(Button, { text: "Add model/experiment", action: this.props.actions.addData }),
          React.createElement(Button, { text: "Cancel", action: () => this.props.actions.setAddData(addDataEnum.none) }),
        );
  }
}
...
class DialogExport extends React.Component {
  render() {
    return React.createElement("div", { className: "vis-export" },
          ...
          React.createElement(Button, { text: "Export", action: this.props.actions.exportData }),
          React.createElement(Button, { text: "Cancel", action: this.props.actions.toggleExportData }),
        );
  }
}
...
class ModelOptions extends React.Component {
  render() {
    return React.createElement("div", { className: "vis-model-options" },
...
class ModelDatasets extends React.Component {
  render() {
    ...
    return React.createElement("div", null,
          React.createElement("p", { className: "vis-datasets-p" }, "Datasets: ",
            this.props.model.get("orig").datasets.map((dset, key) =>
                  React.createElement(Button,
                        { key
                        , text: dset.name
                        , color: this.props.model.getIn([ "datasetsVisibility", key ]).buttonColor
                        , action() {
...
class ModelGraphsets extends React.Component {
  render() {
    ...
    this.props.model.get("orig").graphsets.some((gset, i) =>
      Immutable.List(gset.datasets).equals(this.props.model.get("legendItems"))
          && (graphsetIndex = i) + 1
    );
    ...
    return React.createElement("div", null,
          React.createElement("p", { className: "vis-graphsets-p" }, "Graphsets: ",
                React.createElement("select",
                      { onChange(e) {
                          e.target.value === "c" || self.props.actions.setCurrentGraphset(
                                self.props.modelIndex,
                                Immutable.List(self.props.model.get("orig").graphsets[+e.target.value].datasets)
                              );
                        }
                      ...
                      graphsetIndex === -1
                          ...
                          : null,
                      this.props.model.get("orig").graphsets.map((gset, key) =>
                        React.createElement("option", { key, value: key }, gset.name)
                      ),
                    ),
                this.props.model.get("orig").legend.map(
                      (item, key) =>
...
class LegendItem extends React.Component {
  render() {
    ...
    return React.createElement("span",
          ...
          , onClick(e) { props.actions.toggleLegendItem(props.modelIndex, props.legendIndex) }
          },
...
class Graph extends React.Component {
  render() {
    ...
    const xAxis = this.props.state.xAxis == "" ? null : this.props.state.models.getIn(
          [ xMIndex, "orig" ]
        ).datasets[this.props.state.xDataset].data[xTIndex]; 
        ...
        visibility.hidden === model.getIn([ "datasetsVisibility", dsetIndex ])
          ...
          if (dataIndex && model.getIn([ "legendItems", dataIndex - 1 ])) {
            ...
            data.push(
                ...
                , y
                , yaxis: i % 2 == 0 ? "y" : "y2"
                , name: model.get("orig").legend[dataIndex - 1].name + " (" + dset.name + ")"
    ...
    return React.createElement("div", { className: "vis-graph", style: { zIndex: this.props.background ? -1 : 0 } },
          React.createElement(Plot,
                ...
                , layout:
                  ...
                  , yaxis2:
                    ...
                    , overlaying: "y"
                    , side: "right"
                    , title: this.props.state.models.getIn([ "1", "orig" ])
...
class GlobalSettings extends React.Component {
  render() {
    return React.createElement("div", { className: "global-settings" },
          React.createElement(Button,
          ...
          React.createElement(Button, { text: "Export", action: this.props.actions.toggleExportData }),
          React.createElement("p", { className: "global-settings-out" },
                "Current x axis type: " + this.props.state.axisType.name
                + ". Current interpolation method: " + this.props.state.interpolation.name + ".",
              ),
        );
  }
}
